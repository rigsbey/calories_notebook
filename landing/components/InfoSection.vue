...
const infoBlocks = ref([
  ...
  },
  {
    ...
    content: 'Наш <strong>ИИ-бот</strong> использует технологии <strong>распознавания еды по фото</strong> для определения состава блюда. Бот анализирует ингредиенты, рассчитывает <strong>питание и витамины</strong>, а затем сохраняет данные в <strong>Google Calendar</strong> для отслеживания прогресса.'
  },
  {
    title: '📱 Преимущества Telegram-бота для диеты',
    content: 'В отличие от обычных <strong>приложений для похудения</strong>, наш бот работает прямо в Telegram. Не нужно скачивать дополнительные программы — просто отправляйте фото еды и получайте <strong>автоматический подсчет калорий</strong> с сохранением в календарь.'
  }
])
